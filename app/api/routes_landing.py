...
<body>
    ...
    <div class="container">
      <div class="grid-12">
        ...
        <div class="col-span-9">
          ...
          <p class="section-subtext">
            Trigger end-to-end intelligence execution: Discover PENDING companies, collect independent HTTP and browser signals, judge with Gemini 3.1 Flash-Lite, and write results back to Google Sheets.
          </p>
  ...
  <section class="section-wrap">
    <div class="container">
      <div class="grid-12">
        ...
        <div class="col-span-9">
          <h2 class="section-heading-large">RESULTS &amp; VERDICTS</h2>
          <p class="section-subtext">
            Actual persisted evaluations, calibrated confidence scores, evidence-based reasoning, and discovery questions generated by the LLM judge.
          </p>

          <div id="verdictsList" class="verdicts-list">
            <div class="mono-text" style="color: var(--text-muted); padding: 1.5rem 0;">No evaluated verdicts yet. Run the pipeline to generate decisions.</div>
          </div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    function renderVerdicts(companies) {
      ...
      if (judged.length === 0) {
        container.innerHTML = `<div class="mono-text" style="color: var(--text-muted); padding: 1.5rem 0;">No evaluated verdicts yet. Run the pipeline to generate decisions.</div>`;
        return;
      }
          ...
          <div class="verdict-card">
            <div class="verdict-top">
              ...
              <div class="mono-text" style="font-size:11px; color:var(--text-muted);">
                ${c.sheet_row_id ? `SHEET: ${escapeHtml(c.sheet_row_id)} &bull; ` : ''}EVALUATED: ${v.evaluated_at ? new Date(v.evaluated_at).toLocaleTimeString() : 'RECENT'}
              </div>
            </div>
      ...
      const poll = async () => {
        try {
          ...
          if (status === 'RUNNING') {
            ...
            // Refresh companies and verdicts
            await loadCompanies();
